...
Our database is defined by drizzle-orm in packages/db, when creating new files, follow the naming convention "concept-db.ts", like "users-db.ts", then, make sure to add it to the packages\db\src\schema.ts file. This can be imported from '@guitar-app/db'. If changes are made to the database, do not worry about generating or creating SQL migrations, that will be handled by me.
...
1. We built out a guitar learning client inside of @frontend\components\learn\caged-practice-client.tsx that is used inside of @frontend\components\learn\learn-module-client.tsx at the /dashboard/learn/module route. You enter this from the /dashboard/learn route in the dialog that pops up when you click a topic, if you have learn mode selected. We are now going to add the practice mode. The practice mode will be like the learning mode, but without the lesson pages. Instead, the user is directly taken to the fretboard preview page. They can choose to either show all notes, show roots, or hide all notes. This control if the note is visible before they have played it, just like how it is when the practice client is in its test mode. If the user is in cagedRoots practice mode, they will be able to practice playing the roots over and over. If they are in chord tones, they practice only the chord tones, if they are in minor pentatonic, they practice the minor pentatonic. When they click on a topic on the /dashboard/learn page, in the dialog that pops up, if they choose practice, they can set options for the duration, whether it is based on completed shapes, timed, or infinite, they can also select the key to play. Make sure that these options take effect in our practice mode. This will take them to the /dashboard/practice/[module] route, make sure to set that up so that it handles the caged roots, chord tones, and minor pentatonic.

2. Add a new practice module to the caged pathway. This will be major scale. This is just about adding the 4th and 7th degree notes. The 4th comes right after the 3rd, the 7th right before the root. Make sure this new lesson in the pathway is complete, just as the minor pentatonic is. The modules should be broken up into a major scale intro, where it shows what it is, how to assemble it from the minor pentatonic, and other information. Then, practice the C shape in the key of D, make them run through this shape 3 times, then, show a short article reviewing that shape and introducing the A shape, then make them play the A shape 3 times in the key of D, this repeats for all shapes. Then, test them once per shape in the key of D in the mode where only the root notes show at first, and they have to play the other notes to reveal them. Finally, quiz them with a 5 question quiz. This should all be modeled off the structure and pattern we have established in the minor pentatonic pathway, make sure we also track this in our database fo rthe users lesson tracking.
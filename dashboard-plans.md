...
1. We are going to expand our learning mode which was built for the caged roots inside of @frontend\app\(withAuth)\dashboard\(noNav)\learn\[module]\page.tsx . We are going to now add chord tones, this should have the same format, of a learning article module, then a 2 rounds of learning, with 1 round of testing, then 3 keys, and finally a quiz at the end. This time, instead of only using the roots, we are now going to also have the chord tones in each shape, so the root is the 1, then we also include the 3rd and the 5th degree notes. Again, it should function the same, just with chord tones included now, this should be activated by the card on the /dashboard/learn page for the chord tones card, it should have the same controls on the dialog that opens for the caged roots, there is a learn mode and practice mode. The practice mode just redirects to the /dashboard/practice/scales page, do not worry about that implementation, you are going to focus on the cagedChordTones module.

2. We are going to expand our learning mode which was built for the caged roots and chord tones inside of @frontend\app\(withAuth)\dashboard\(noNav)\learn\[module]\page.tsx . We are going to now add the minor pentatonic, this should have the same format, of a learning article module, then a 2 rounds of learning, with 1 round of testing, then 3 keys, and finally a quiz at the end. This time, instead of only using the roots and chord tones, we are now going to also have the minor pentatonic shapes, so the root is the 1, we are already including the 3 and 5 from the chord tones, and now we also add in the 2nd and 6th degree notes, forming a minor pentatonic scale. Again, it should function the same, just with the new notes added, this should be activated by the card on the /dashboard/learn page for the minor penatatonic card, it should have the same controls on the dialog that opens for the caged roots and chord tones, there is a learn mode and practice mode. The practice mode just redirects to the /dashboard/practice/scales page, do not worry about that implementation, you are going to focus on the cagedMinorPentatonic module.

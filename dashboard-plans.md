...
1. We have a guitar learning and practice client to help the user learn their scales. We need to give the user more control while they are in their practice mode. Right now, the user enters practice mode from the dashboard in @frontend\app\(withAuth)\dashboard\(withNav)\learn\page.tsx and in @frontend\app\(withAuth)\dashboard\(withNav)\practice\page.tsx. From the learn page, we show them a dialog, that can be set to the practice tab, this practice tab is the same as what is shown on the practice page. We are going to add a new control to let them only play certain scale degrees (1, 2, 3, 4, 5, 6, 7). On both the learn and practice page, we will only add this control to the major scale. Then, when they start practicing, it should only show and require that they play the notes they selected in the control. By default this control will have all degrees selected (1-7). If they removed degrees 4 and 6, then when they practice they will still play all shapes as they normally do, just without having to play the 4 and 6 degree notes.